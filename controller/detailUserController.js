	...
	try {
		...
		if (!(id_user && job_title && address && job_type && description)) {
			...
			if (getData.rowCount) {
				...
				const postData = await model.addedDetailUsers(id_user, fixjob_title, fixaddress, fixjob_type, fixdescription);
				res.status(200).send({
...
const updateDetailUser = async (req, res) => {
	try {
		...
		if (getData?.rowCount) {
			...
			await model.updateDetailUser({ id_user, changeJobTitle, changeAddress, changeJob_type, changeDescription });
			res.send({
