...
const addedDetailUsers = (id_user, fixjob_title, fixaddress, fixjob_type, fixdescription) => {
	return new Promise((resolve, reject) => {
		db.query(
...
const getAllDataUser = () => {
	return new Promise((resolve, reject) => {
		db.query(`SELECT * FROM detailuser INNER JOIN registeruser ON detailuser.id_user = registeruser.id`, (error, result) => {
			if (error) {
				reject(error);
			} else {
				resolve(result);
...
const updateDetailUser = (props) => {
	const { id_user, changeJobTitle, changeAddress, changeJob_type, changeDescription } = props;
	return new Promise((resolve, reject) => {
		db.query(
			`UPDATE detailuser SET job_title = $1, address = $2, job_type= $3, description = $4 WHERE id_user = $5`,
			[changeJobTitle, changeAddress, changeJob_type, changeDescription, id_user],
			(error, result) => {
				if (error) {
					reject(error);
				} else {
